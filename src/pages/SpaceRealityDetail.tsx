  ...
  sections: [
    {
      ...
      video: 'https://files.catbox.moe/k8gnsk.mp4',
      points: [
        'Integrated Firebase-based OTP login system inside Unity using WebView.',
        'Reused the WebView OTP verification flow originally developed for Meiphor and customized for Space Reality.',
        'Enabled auto-login detection and scene reload after successful authentication.'
      ]
    },
    {
      heading: 'Serial Key Activation',
      video: 'https://files.catbox.moe/nlsk5w.mp4',
      points: [
        'Implemented serial key–based activation system for secure AR content access.',
        'Optimized UI for smooth profile editing and key verification experience',
        'Designed Edit Profile section to allow users to enter their serial key and activate instantly',
        'Integrated backend validation to verify key authenticity and prevent duplicate activations',
        'Synced activation status with user profile in real-time for a seamless unlock process'
      ]
    },
    {
      ...
      points: [
        ...
        'Demonstration of real-time asset loading in the Space Reality app',
...
export const SpaceRealityDetail = () => {
      ...
      if (exploreFeaturesElement) {
        ...
        if (exploreRect.top > 0) {
          setActiveSection('');
            ...
            const sections = spaceRealityData.sections.map((_, index) => `section-${index}`);
            ...
            >
              <div className="text-center mb-4 -mt-32">
                <h3 className="text-2xl font-semibold text-white">
                  {spaceRealityData.sections.find(section => section.video === panelVideoSrc.split('?')[0])?.heading || 'Video'}
                </h3>
              </div>
        ...
        <div className="container mx-auto px-1 py-2 relative z-10">
          ...
          >
            <div className="flex flex-col items-center mb-4">
              <motion.div
                className="text-6xl text-center flex flex-row items-center justify-center gap-6 mt-12"
                style={{ marginBottom: 8 }}
              >
                <img 
                  key="space-reality-logo-detail"
                  src="https://i.postimg.cc/k4kqt5wx/SPACE-REALITY-3-Copy.png" 
                  alt="Space Reality Logo"
                  className="w-32 h-32 object-contain ml-12"
                />
                <h1 className="text-5xl font-bold text-foreground tracking-wider ml-8">
                  SPACE <span className="text-neon-green">REALITY</span>
                </h1>
              </motion.div>
              <p className="text-lg md:text-xl text-muted-foreground mb-12 max-w-full mx-auto px-1 text-justify font-medium leading-relaxed tracking-normal mt-4 md:mt-12" style={{fontFamily: 'Tahoma, sans-serif', wordSpacing: '0.3em', textIndent: '2.25em'}}>
              Led the development of Space Reality, an AR-based educational app that brings space and astronomy concepts to life through immersive 3D experiences. Integrated client-provided models and custom-built interactions to ensure smooth, high-performance output across Android and iOS platforms. Refined several interaction and system patterns from previous AR projects to maintain consistency, scalability, and a seamless user experience.
              </p>
              
              {/* Download Buttons */}
              <div className="flex flex-wrap justify-center gap-6 mb-6">
                <motion.button
                  onClick={() => window.open(spaceRealityData.links.playStore, '_blank')}
                ...
                <motion.button
                  onClick={() => window.open(spaceRealityData.links.appStore, '_blank')}
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-2 px-6 py-3 bg-neon-green text-background rounded-lg hover:bg-neon-green/90 transition-all duration-300 font-medium"
                >
              ...
              <div className="w-full py-2">
                <div className="flex items-center justify-end">
                  <div className="relative" data-section-menu>
                    ...
                    >
                      <div className="py-2">
                        {spaceRealityData.sections.map((section, index) => (
                          <motion.button
                            key={index}
                            onClick={() => {
          ...
          {spaceRealityData.sections.map((section, index) => (
            <div key={index}>
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                className={index === spaceRealityData.sections.length - 1 ? "mb-4" : "mb-16"}
              ...
              >
                ...
                {section.video ? (
                  section.video.endsWith('.mp4') ? (
                    <div className="mb-6 rounded-xl overflow-hidden border border-neon-green/10 max-w-[720px] mx-auto">
                      <video
                        controls
                ...
                ) : section.heading === 'WebView OTP Verification' ? (
                  <div className="grid grid-cols-1 gap-4 ml-16 md:ml-48 lg:ml-64">
                    {section.points.map((point, pointIndex) => (
                      <motion.div
                        key={pointIndex}
